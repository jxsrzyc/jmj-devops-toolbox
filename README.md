...
- 🔧 **用户列表创建时间格式化**：MySQL DATETIME 经 jsonify 输出的 RFC1123 GMT 串（如 `Wed, 05 Aug 2026 02:15:20 GMT`）改为显示 `2026-09-04 10:08:17` 格式。注意：值即数据库原值（北京时间），仅去掉 GMT 标签，**不做时区换算**（复用既有 `formatDateTime`，UTC 字段提取 = 取原值）
...
- ✏️ **登录框品牌区简化**：移除品牌副标题 `OPERATION · CI/CD · AI`，品牌区仅保留中文「运维工具箱」五个字占满
